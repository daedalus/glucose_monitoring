...
def png_to_pdf(png_path, pdf_path):
    ...
    The PDF page size is set to match the PNG dimensions at the PNG's stored
    DPI (defaulting to 96 DPI when not present) so the image is not resampled.
    ...
    pdf.image(png_path, x=0, y=0, w=width_mm, h=height_mm)
    pdf.output(pdf_path)
